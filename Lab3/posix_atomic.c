...
void* thread_func(void *data) {
     ...
     while(atomic_load_explicit(&global_counter, memory_order_seq_cst) < SIZE) {
          ...
          if(length < temp_data -> radius) {
               atomic_fetch_add_explicit(&global_success, 1, memory_order_seq_cst);
               temp_data -> thread_success++;
          }

          atomic_fetch_add_explicit(&global_counter, 1, memory_order_seq_cst);
